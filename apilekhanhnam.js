...
const API_KEY = "bolekhanhhuyvidai-apilc79vippro"; // Thay đổi key này bằng key của bạn
...
fastify.addHook("onRequest", async (request, reply) => {
  if (request.url.startsWith("/api/sunwin") || request.url.startsWith("/api/history-json")) {
    ...
    if (!urlKey || urlKey !== API_KEY) {
      return reply.code(403).send({ error: "Key sai mẹ rồi, liên hệ tele: @nguoilbantool" });
    }
  }
});
